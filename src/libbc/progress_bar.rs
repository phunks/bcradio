...
use crate::ceil;
use crate::libbc::player::{PROG, PARK};
...
pub(crate) trait Progress<'a> {
    ...
    fn get_progress_bar_progress_info(&self, elapsed_seconds: u64, total_seconds: Option<u64>) -> String;
...
impl Clone for Bar<'_> {
    fn clone(&self) -> Self {
        Self {
            mutex_progress_bar: Arc::new(Mutex::from(self.mutex_progress_bar.lock().unwrap().clone())),
            phantom: PhantomData,
        }
    }
}
...
impl Progress<'static> for Bar<'static> {
    ...
    fn update_song_info_on_screen(&self, item: CurrentTrack) -> Result<()> {
        ...
        println!("{}       {}", "Song:".truecolor(146, 49, 176), item.track);
        println!("{}     {}", "Artist:".truecolor(126, 87, 194), item.artist_name);
        println!("{}      {}", "Album:".truecolor(121, 134, 203), item.album_title);
        ...
        let progress_bar_style =
            ProgressStyle::with_template("{prefix}  {wide_bar} {progress_info} {spinner:.dim.bold} ")?
                .tick_chars("⠁⠂⠄⡀⠄⠂ ")
            .with_key(
                    "progress_info",
                    move |state: &ProgressState, write: &mut dyn Write| {
                        let progress_info =
                            b.get_progress_bar_progress_info(state.pos(), state.len());
    ...
    fn get_progress_bar_current_position(&self) -> Duration {
        PROGRESS_BAR.lock().unwrap().clone().unwrap().eta()
    }
    ...
    fn enable_tick_on_screen(&self) {
        ...
        PROGRESS_BAR.lock().unwrap().clone().unwrap().enable_steady_tick(interval);
    }

    fn disable_tick_on_screen(&self) {
        PROGRESS_BAR.lock().unwrap().clone().unwrap().disable_steady_tick();
    }
    ...
    fn enable_spinner(&self) {
        if *PROG.lock().unwrap() {
            ...
            a.clone().unwrap().enable_steady_tick(Duration::from_millis(100));
    ...
    fn disable_spinner(&self) {
        if *PROG.lock().unwrap() {
            let a = PROGRESS_BAR.lock().unwrap();
            a.clone().unwrap().disable_steady_tick();
